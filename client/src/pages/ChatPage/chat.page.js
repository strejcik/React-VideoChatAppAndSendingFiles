...
const ChatPage = ({user}) => {
    ...
    useEffect(() => {
        ...
        socket.current.on("getUsers", users=>{
            setOnlineUsers(users.filter(f => user.followings.find(u => f.userId === u? [f.userId, f.socketId] : null )));
        });
    ...
    useEffect(() => {
        ...
        socket.current.emit("refreshOnlineUsers", () => {
            socket.current.on("getUsers", users=>{
                 setOnlineUsers(user.followings.filter(f=> users.find(u => u.userId === f ? [f.userId, f.socketId] : null )));
            });
        });
        
    }, [currentChat, arrivalMessage, user]);
    ...
    useEffect(() => {
      ...
      return () => clearInterval(refreshUsers);
    ...
    //============Video useEffect start============




    useEffect(() => {
        socket.current.emit('getMe', (user._id));
            socket.current.on("me", (i) => {
            setMe(i);
		});
        socket.current.on("hey", (data) => {
            setReceivingCall(true);
            ringtoneSound.play();
            console.log("running caller", data.from);
            setCaller(data.from);
    ...
    let acceptCall = () => {
      ...
      .catch(()=>{
        setModalMessage('You cannot place / receive a call without granting video and audio permissions')
  ...
  let UserVideo;
    ...
    return(
        <React.Fragment>
            ...
            {incomingCall}
            <>{modalMessage && modalVisible && <div className={'videoModalPremission'} onClick={() => {setModalVisible(false); setModalMessage("");}}>{modalMessage}</div>}</>
            ...
            { !incomingCall && !callAccepted && !callingFriend &&<Chat
                //====Chat props start====//
